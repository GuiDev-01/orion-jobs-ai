...
class TestAdzunaService(unittest.TestCase):
    ...
    def test_fetch_adzuna_jobs_success(self, mock_get):
        # Setup mock response para sucesso
    ...
    @patch('app.services.adzuna_service.requests.get')
    def test_fetch_adzuna_jobs_error(self, mock_get):
        # Setup mock to raise exception
        mock_get.side_effect = Exception("API error")
        
        # Call the function - deve retornar lista vazia sem lançar exceção
        result = fetch_adzuna_jobs(country="gb", query="python")
    ...
    def test_normalize_adzuna_jobs(self):
        ...
        # Assert normalization        self.assertEqual(len(normalized), 1)
